...
class LocalPlanner:
    ...
    def get_goal_state_set(self, goal_index, goal_state, waypoints, ego_state):
        ...
        # print(heading, len(waypoints),goal_index , waypoints[goal_index+1][0] , waypoints[goal_index][0])
...
# def main():
#     ego_state = [0,0,np.pi/2]
#     waypoints = [[0,0],[0,1],[0,2],[1,3],[1,4],[1,5],[1,6],[1,7],[0,8],[0,9]]

#     local_planner = LocalPlanner(5,0.1)

#     paths = local_planner.get_goal_state_set(5,[1,4],waypoints,ego_state)

#     path = local_planner.plan_paths(paths)
...
#     if len(path[0])>0:
#         for i in range(len(path[0])):
#             plt.plot(path[0][i][0], path[0][i][1], '-')    
#         plt.show()
#     else:
#         print(path[1])
