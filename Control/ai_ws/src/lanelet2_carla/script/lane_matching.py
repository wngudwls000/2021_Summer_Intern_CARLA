...
matches = lanelet2.matching.getDeterministicMatches(lanelet_map, obj, 10000000.)  # max distance = 4m   11544000
...
# current_lanelet = matches[0].lanelet.id
# print("lanelet number:",current_lanelet)
...
while True:
    ...
    print("line_start_end: ",point_pair_list)
    map_file = open("Town01.osm")
    ...
    for list_index in range(len(start_end_list)):
        for line_index in range(start_end_list[list_index][0],start_end_list[list_index][1]+1):
            ...
            # x,y = gps2plane2(lat-38.0,lon-125.0)
            y *=-1.0
    ...
    # for i in range(len(point_num_list)):

    #     cur_point_num = point_num_list[i]
    ...
    #             if  line_split[2]==str(cur_point_num):
