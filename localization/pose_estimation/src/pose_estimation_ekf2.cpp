...
void EKF::GNSS_Callback(const sensor_msgs::NavSatFix::ConstPtr &msg)
{
    ...
    // geometry_msgs::PoseStamped local_pose = geodetic_to_enu(lat_raw, lon_raw, hei_raw, 0.0, 0.00000898315002, 2.00000008);
...
{
    double lamb = lat * D_DEG_2_RAD;
    double phi = lon * D_DEG_2_RAD;
    double s = sin(lamb);
    double N = a / sqrt(1 - e_sq * s * s);
    geodetic_to_ecef_result <<      (h + N) * cos(lamb) * cos(phi),
                                    (h + N) * cos(lamb) * sin(phi),
                                    (h + (1 - e_sq) * N) * sin(lamb);
...
{
    double lamb = lat0 * D_DEG_2_RAD;
    double phi = lon0 * D_DEG_2_RAD;
    double s = sin(lamb);
    double N = a / sqrt(1 - e_sq * s * s);
    double x0 = (h0 + N) * cos(lamb) * cos(phi);
    double y0 = (h0 + N) * cos(lamb) * sin(phi);
    double z0 = (h0 + (1 - e_sq) * N) * sin(lamb);
    double xd = x - x0;
    double yd = y - y0;
    double zd = z - z0;
    ecef_to_enu_result <<   (-1) * sin(phi) * xd + cos(phi) * yd,
                            (-1) * cos(phi) * sin(lamb) * xd - sin(lamb) * sin(phi) * yd + cos(lamb) * zd,
                            cos(lamb) * cos(phi) * xd + cos(lamb) * sin(phi) * yd + sin(lamb) * zd;
